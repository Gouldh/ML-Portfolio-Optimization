...
def download_factor_data(start_date, end_date):
    ...
    for factor, tickers in factor_proxies.items():
        if isinstance(tickers, str):
            ...
            data = yf.download(tickers, start=start_date, end=end_date, progress=False)['Close'].pct_change()
            factor_data[factor] = data
        else:
            # Difference between two tickers (e.g., SMB, HML)
            data1 = yf.download(tickers[0], start=start_date, end=end_date, progress=False)['Close'].pct_change()
            data2 = yf.download(tickers[1], start=start_date, end=end_date, progress=False)['Close'].pct_change()
